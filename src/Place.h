...
public:
  ...
  // test place types
  bool is_household() {
    return type == Place::HOUSEHOLD;
  }
  
  bool is_neighborhood() {
    return type == Place::NEIGHBORHOOD;
  }
  
  bool is_school() {
    return type == Place::SCHOOL;
  }
  
  bool is_classroom() {
    return type == Place::CLASSROOM;
  }
  
  bool is_workplace(){
    return type == Place::WORKPLACE;
  }
  
  bool is_office() {
    return type == Place::OFFICE;
  }
  
  bool is_hospital() {
    return type == Place::HOSPITAL;
  }
  
  bool is_community() {
    return type == Place::COMMUNITY;
  }

  // test place subtypes
  bool is_college(){
    return subtype == fred::PLACE_SUBTYPE_COLLEGE;
  }
  
  bool is_prison(){
    return subtype == fred::PLACE_SUBTYPE_PRISON;
  }
  
  bool is_nursing_home(){
    return subtype == fred::PLACE_SUBTYPE_NURSING_HOME;
  }
  
  bool is_military_base(){
    return subtype == fred::PLACE_SUBTYPE_MILITARY_BASE;
  }
   ...
   */
  void set_patch(Neighborhood_Patch *p) { this->patch = p; }
  ...
  bool has_infectious_vectors(int disease_id) { return I_vectors[disease_id] > 0; }
...
protected:
  ...
  struct Allocator {
    ...
    bool reserve( int n = 1 ) {
      if(this->remaining_allocations == 0) {
