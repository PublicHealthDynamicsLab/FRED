...
class Tracker {
 public:
  ...
  string get_index_name(void) {return this->index_name;}
  T get_index_value(int i) {return this->indices[i];}
...
#pragma omp atomic
  ...
  void reset_all_index_key_pairs_to_zero(void) {
    for (int i = 0; i < this->indices.size(); ++i){
      reset_index_all_key_pairs_to_zero(indices[i]);
    }
  }
  ...
  string print_key_index_list(string key_name){
    ...
    if(key_type == "None"){
      ...
      if (this->indices.size() != values_map_int[key_name].size())
	ERROR_PRINT("Tracker.h::print_key_index_list there is something wrong with the counts number of indices != number of values for key %s\n",
      ...
      for ( int i = 0; i < this->indices.size(); ++i){
	returnString << this->indices[i] << "\t\t" << values_map_int[key_name][i] << "\n";
      }
    }
    else if(key_type == "double"){
	...
	ERROR_PRINT("Tracker.h::print_key_index_list there is something wrong with the counts number of indices != number of values for key %s\n",
      ...
      for ( int i = 0; i < this->indices.size(); ++i){
	returnString << this->indices[i] << "\t\t" << values_map_double[key_name][i] << "\n";
      }
    }
    else if(key_type == "string"){
      if (this->indices.size() != values_map_string[key_name].size())
	ERROR_PRINT("Tracker.h::print_key_index_list there is something wrong with the counts number of indices != number of values for key %s\n",
		    key_name.c_str());
      
      for ( int i = 0; i < this->indices.size(); ++i){
	returnString << this->indices[i] << "\t\t" << values_map_string[key_name][i] << "\n";
      }
    }
    else {
