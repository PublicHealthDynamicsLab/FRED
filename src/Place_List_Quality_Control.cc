...
void Place_List::report_school_distributions(int day) {
  ...
  for (int c = 0; c <= 20; c++) { count[c] = 0; osize[c] = 0; nsize[c] = 0; }
  for (int p = 0; p < number_places; p++) {
    if (places[p]->get_type() == Place::SCHOOL) {
      int os = places[p]->get_orig_size();
      int ns = places[p]->get_size();
      int n = os / 50;
      if (n > 20) n = 20;
      count[n]++; osize[n] += os; nsize[n] += ns;
    }
  }
...
void Place_List::report_household_distributions() {
  ...
  if (Global::Verbose) {
    ...
    total = 0;
    // size distribution of households
    for (int c = 0; c <= 10; c++) { count[c] = 0; hsize[c] = 0; }
    for (int p = 0; p < number_places; p++) {
      if (places[p]->is_household()) {
        int n = places[p]->get_orig_size();
	int hs = places[p]->get_size();
        if (n <= 10) { count[n]++; hsize[n] += hs; }
        else { count[10]++; hsize[10] += hs; }
        total++;
      }
    }
...
void Place_List::quality_control() {
  ...
  if (Global::Verbose) {
	...
	hsize += n;
      ...
      if (places[p]->get_type() == Place::NEIGHBORHOOD) {
	nn++;
	nsize += n;
      }
      if (places[p]->get_type() == Place::SCHOOL) {
  ...
  if (Global::Verbose) {
    ...
    for (int p = 0; p < number_places; p++) {
      if (places[p]->get_type() == Place::SCHOOL) {
        // places[p]->print(0);
        for (int c = 0; c < 100; c++) {
          count[c] += ((School *) places[p])->get_students_in_grade(c);
        }
      }
    }
    for (int c = 0; c < 100; c++) {
  ...
  if (Global::Verbose) {
    ...
    for (int p = 0; p < number_places; p++) {
      if (places[p]->get_type() == Place::CLASSROOM) {
        int s = places[p]->get_size();
        int n = s;
        if (n < 50) { count[n]++; }
        else { count[50-1]++; }
  ...
  if (Global::Verbose) {
    ...
    for (int c = 0; c <= 100; c++) { count[c] = 0; }
    for (int p = 0; p < number_places; p++) {
      if (places[p]->get_type() == Place::WORKPLACE || places[p]->get_type() == Place::SCHOOL) {
        int s = places[p]->get_size();
	if (places[p]->get_type() == Place::SCHOOL) {
...
if (Global::Verbose) {
  ...
  for (int p = 0; p < number_places; p++) {
    if (places[p]->get_type() == Place::OFFICE) {
      int s = places[p]->get_size();
      int n = s;
      if (n < 60) { count[n]++; }
      else { count[60-1]++; }
