...
void County::setup() {
  ...
  Params::get_param("population_growth_rate", &(this->population_growth_rate));
  Params::get_param("mortality_rate_multiplier", &mortality_rate_multiplier);
  Params::get_param("birth_rate_multiplier", &birth_rate_multiplier);
  Params::get_param("mortality_rate_adjustment_weight", &(this->mortality_rate_adjustment_weight));
  Params::get_param("college_departure_rate", &(this->college_departure_rate));
  Params::get_param("military_departure_rate", &(this->military_departure_rate));
  Params::get_param("prison_departure_rate", &(this->prison_departure_rate));
  Params::get_param("youth_home_departure_rate", &(this->youth_home_departure_rate));
  Params::get_param("adult_home_departure_rate", &(this->adult_home_departure_rate));
  ...
  Params::get_param("enable_migration_to_target_popsize", &(County::enable_migration_to_target_popsize));
  Params::get_param("enable_county_to_county_migration", &(County::enable_county_to_county_migration));

  sprintf(param_name, "birth_rate_file_%d", fips);
  strcpy(birth_rate_file, "");
  Params::get_param(param_name, birth_rate_file);
  if (strcmp(birth_rate_file,"") == 0) {
    Params::get_param("birth_rate_file", birth_rate_file);
  }

  sprintf(param_name, "mortality_rate_file_%d", fips);
  strcpy(mortality_rate_file, "");
  Params::get_param(param_name, mortality_rate_file);
  if (strcmp(mortality_rate_file,"") == 0) {
    Params::get_param("mortality_rate_file", mortality_rate_file);
  }

  Params::get_param("enable_within_state_school_assignment", &(County::enable_within_state_school_assignment));
  ...
  Global::Places.get_country_directory(country_dir);
  sprintf(male_target_file,
	  "%s/projections/%d/%d-males.txt",
	  country_dir, state_fips, this->fips);

  sprintf(female_target_file,
	  "%s/projections/%d/%d-females.txt",
	  country_dir, state_fips, this->fips);
...
void County::read_migration_parameters() {
  ...
  Params::disable_abort_on_failure();
  Params::get_param("county_migration_file", county_migration_file);
  Params::set_abort_on_failure();
...
void County::read_population_target_parameters() {
  ...
  ///
  Params::disable_abort_on_failure();
  Params::get_param("migration_file", migration_file);
  Params::set_abort_on_failure();
