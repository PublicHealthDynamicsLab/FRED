...
Visualization_Layer::Visualization_Layer() {
  ...
  if (gaia_mode) {
    ...
    this->rows = (double)(this->max_y - this->min_y) / patch_size;
    if (this->min_y + this->rows * patch_size < this->max_y) this->rows++;
    this->cols = (double)(this->max_x - this->min_x) / patch_size;
    if (this->min_x + this->cols * patch_size < this->max_x) this->cols++;
    ...
    grid = new Visualization_Patch * [this->rows];
    for (int i = 0; i < this->rows; i++) {
      grid[i] = new Visualization_Patch[this->cols];
    }

    for (int i = 0; i < this->rows; i++) {
      for (int j = 0; j < this->cols; j++) {
	grid[i][j].setup(i, j, this->patch_size, this->min_x, this->min_y);
      }      
    }
  }
}
...
void Visualization_Layer::create_data_directories(char * vis_top_dir) {
  ...
  for (int d = 0; d < Global::Diseases; d++) {
    ...
    if (d == 0) {
      ...
      FILE *fp = fopen(filename, "w");
      int num_households = households.size();
      for(int i = 0; i < num_households; ++i) {
	Place * h = households[i];
