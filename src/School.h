...
public:
  ...
  School(const char*, fred::place_subtype subtype, double, double, Place *, Population *pop);
  void prepare();
  ...
  int get_max_grade() { return max_grade; }
  int get_orig_students_in_grade(int grade) {
    if (grade < 0 || max_grade < grade) { return 0; }
    return orig_students_in_grade[grade];
  }
  int get_students_in_grade(int grade) {
    if (grade < 0 || max_grade < grade) { return 0; }
    return students_in_grade[grade];
  }
  int get_classrooms_in_grade(int grade) {
    if(grade < 0 || GRADES <= grade) {
      return 0;
    }
    return (int)this->classrooms[grade].size();
  }
  ...
  int get_orig_number_of_students() { 
    int n = 0;
    for (int grade = 1; grade < GRADES; grade++) {
      n += orig_students_in_grade[grade];
    }
    return n;
  }
